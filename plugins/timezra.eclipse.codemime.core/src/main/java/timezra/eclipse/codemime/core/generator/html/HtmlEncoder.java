...
public class HtmlEncoder {
	...
	private static final String[][] ISO8859_1_CHARACTERS = { { "nbsp", "160" }, { "iexcl", "161" }, { "cent", "162" },
	...
	private static final String[][] HTML40_CHARACTERS = { { "fnof", "402" }, { "Alpha", "913" }, { "Beta", "914" },
	...
	public String encode(final String html) {
		...
		for (int i = 0; i < html.length(); i++) {
			final char chr = html.charAt(i);

			final String entity = entityMap.get((int) chr);

			if (entity == null) {
				if (chr > 0x7F) {
					encodedHtml.append("&#");
					encodedHtml.append(Integer.toString(chr, 10));
					encodedHtml.append(';');
				} else {
					encodedHtml.append(chr);
				}
			} else {
				encodedHtml.append('&');
				encodedHtml.append(entity);
				encodedHtml.append(';');
			}
		}
